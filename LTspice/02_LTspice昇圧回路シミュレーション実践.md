...
10. [まとめ](#まとめ)
11. [理想スイッチからNMOSへの変更](#理想スイッチからnmosへの変更)
12. [ゲート駆動回路の役割（詳細解説）](#ゲート駆動回路の役割詳細解説)
13. [演習課題：実際の部品を使ってシミュレーション](#演習課題実際の部品を使ってシミュレーション)
...
⚠️ 理論値と実測値の違いに注目！
デューティ比0.5の場合、昇圧率は理論上2倍（5V → 10V）
実際の出力が理論値より大きい場合、回路のどこかに問題がある可能性があります。
```
...
D = 0.5の場合:
昇圧率 = 1 / (1 - 0.5) = 1 / 0.5 = 2倍
期待される出力電圧 = 5V × 2 = 10V
```

**確認すべき波形:**

以下の波形を測定して、回路の動作を詳しく調べてみましょう:

1. **V2の波形（PWM信号）**: デューティ比が本当に0.5か確認
2. **V(sw_node)の波形**: スイッチング動作が正しいか確認
3. **I(L1)の波形**: インダクタ電流の変化を確認
4. **V(vout)の波形**: 出力電圧が安定しているか確認
5. **I(D1)の波形**: ダイオード電流の流れ方を確認

💡 **調査のヒント**:
- 波形測定は回路図上のノードや部品を直接クリック
- 複数の波形を重ねて表示して、タイミング関係を確認
- カーソル機能（右クリック→Cursor）で正確な値を読み取る
- 定常状態の波形を見るため、シミュレーション時間の後半を観察
...
## 1️⃣1️⃣ 理想スイッチからNMOSへの変更

### なぜNMOSに変更するのか？

これまで使用していた理想スイッチ（SW）は、学習目的には便利ですが、実際の回路ではMOSFETを使用します。より実践的なシミュレーションを行うため、理想スイッチをNチャネルMOSFET（NMOS）に置き換えます。

**理想スイッチとNMOSの違い:**

| 項目 | 理想スイッチ（SW） | NMOS |
|------|------------------|------|
| **モデル** | `.model`定義が必要 | LTspice内蔵モデル |
| **ON抵抗** | 任意に設定 | 実際のMOSFET特性 |
| **ゲート制御** | しきい値電圧のみ | ゲート電圧に依存 |
| **実用性** | 学習用 | 実回路に近い |

### ステップ1: 理想スイッチを削除

1. 回路図上の **S1（理想スイッチ）** をクリックして選択
2. **Delete** キーを押して削除
3. 同時に `.model SW SW(Ron=1m Roff=1G Vt=0.01)` のSPICEディレクティブも削除（不要になるため）

### ステップ2: NMOSを配置

![NMOS選択ダイアログ](02_figure/19_component_dialog_nmos.png)

**図: Component選択ダイアログでNMOSを検索**

1. **P** キーを押して部品選択ダイアログを開く
2. **Search:** 欄に `nmos` と入力
3. リストから **nmos** を選択（N-Channel MOSFET transistor）
4. **Place** ボタンをクリック
5. 元のスイッチがあった位置に配置

💡 **TIPS**: NMOSの端子は以下の3つです：
- **G（ゲート）**: PWM信号を入力
- **D（ドレイン）**: インダクタ・ダイオード側に接続
- **S（ソース）**: GND側に接続

### ステップ3: 配線を接続

NMOSを配置したら、以下のように配線します：
...
実際の回路では、MOSFETのゲートを直接駆動するのではなく、**ゲート抵抗**と**プルダウン抵抗**を追加します。

#### 追加する部品

| 部品 | 値 | 役割 |
|------|-----|------|
| **R2（ゲート抵抗）** | 10Ω | ゲート電流制限・発振防止 |
| **R3（プルダウン抵抗）** | 10kΩ | ゲートをGNDに引き下げ・誤動作防止 |
...
![NMOSを使用した昇圧回路](02_figure/20_boost_circuit_with_nmos.png)

**図: NMOSとゲート駆動回路を追加した昇圧回路**
...
図の回路では、V2に以下のPULSE設定がされています：

```
PULSE(0 10 0 10n 10n 0.005m 0.01m)
```

| パラメータ | 値 | 説明 |
|-----------|-----|------|
| **Vinitial** | 0V | 初期電圧（LOW） |
| **Von** | 10V | パルス電圧（HIGH） |
| **Tdelay** | 0 | 遅延時間 |
| **Trise** | 10ns | 立ち上がり時間 |
| **Tfall** | 10ns | 立ち下がり時間 |
| **Ton** | 0.005ms (5µs) | ONパルス幅 |
| **Tperiod** | 0.01ms (10µs) | 周期 |
...
図の回路では `.tran 100m` と設定されています（100msのトランジェント解析）。

---

## 1️⃣2️⃣ ゲート駆動回路の役割（詳細解説）

### R2（ゲート抵抗）の役割

| 機能 | 説明 |
|------|------|
| **ゲート電流制限** | MOSFETのゲート容量への充放電電流を制限 |
| **高周波発振防止** | ゲート配線のインダクタンスとの共振を抑制 |
| **スイッチング速度調整** | 抵抗値を大きくすると遅くなる |
| **ドライバ保護** | 過大な電流からドライバICを保護 |

**推奨値:** 10Ω〜100Ω（用途により調整）

### R3（プルダウン抵抗）の役割

| 機能 | 説明 |
|------|------|
| **確実なOFF状態** | PWM信号がOFF時にゲートを0Vに保持 |
| **誤動作防止** | ゲート浮遊によるノイズ起因の誤ON防止 |
| **電源投入時の保護** | マイコン起動前にMOSFETがOFFを維持 |
| **ノイズ耐性向上** | 外来ノイズに対する耐性を向上 |
